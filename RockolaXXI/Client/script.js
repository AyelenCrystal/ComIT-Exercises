...
xhr.open("GET", "http://localhost:8379/discos");
// Envío el request
xhr.send();
...
function crearDivDisco(datosDisco) {
    ...
    //Creo el Item Video
    let itemVideo = document.createElement("li");
    
    // Creo el elemento a
    let linkVideo = document.createElement("a");
    linkVideo.href = datosDisco.video;
    linkVideo.target = "_blank";
    linkVideo.textContent = "Ver Video";

    // Agrego el <a> dentro del <li>
    itemVideo.appendChild(linkVideo);
    // Agrego el <li> dentro del <ul>
    listaDatos.appendChild(itemVideo);

    // Agrego el <ul> dentro del <div> contenedor
...
function crearDivDiscoV2(datosDisco) {
    ...
    html += `<li><a href="${datosDisco.Video}" target="_blank"> Ver Video </a><>/li>`;
    html += `</ul>`;
    html += `</div>`;
